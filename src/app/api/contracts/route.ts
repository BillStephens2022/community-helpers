...
export async function POST(req: NextRequest) {
  ...
  try {
    const { worker, client, jobCategory, jobDescription, amountDue }: ContractBody = await req.json();
    ...
    await User.findByIdAndUpdate(worker, { $push: { contracts: newContract._id } });
    await User.findByIdAndUpdate(client, { $push: { contracts: newContract._id } });
